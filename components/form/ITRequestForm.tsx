...
import { cn, generateFormNumber, buildFormNumber} from '@/lib/utils';
import { getNextFormNumber } from '@/lib/actions/form-number';
...
export default function ITRequestForm() {
 ...
 useEffect(() => {
  const fetchNextNumber = async () => {
    try {
      const formNumber = await getNextFormNumber(); // langsung dapat string
      setData(p => ({ ...p, formNumber }));
    } catch (err) {
      console.error('getNextFormNumber error:', err);
      setData(p => ({ ...p, formNumber: buildFormNumber(175) }));
    } finally {
  ...
  const grid2 = 'grid grid-cols-1 sm:grid-cols-2 gap-x-6';
    ...
    return (
      <div className="max-w-4xl mx-auto">
        ...
        <div className={grid2}>
          <Row label="Nomor Form">
            <input value={data.formNumber} disabled onChange={e => u('formNumber', e.target.value)} className={inp + ' cursor-not-allowed opacity-60'} />
          </Row>
        ...
        <Row label="Company" required>
          <input value={data.reqCompany} onChange={e => u('reqCompany', e.target.value)} className={inp + ' cursor-not-allowed opacity-60'} />
        </Row>
        <Row label="Personnel Name" required>
          <input value={data.reqName} onChange={e => u('reqName', e.target.value)} className={inp} placeholder="Nama lengkap requester" />
        </Row>

        {/* Recipient */}
        <Sec icon="📦" title="Recipient Information" bg="bg-sky-50" color="text-sky-800" />
        <div className={grid2}>
          <Row label="Company"><input value={data.recCompany} disabled onChange={e => u('recCompany', e.target.value)} className={inp + ' cursor-not-allowed opacity-60'} /></Row>
          <Row label="Department"><input value={data.recDept} onChange={e => u('recDept', e.target.value)} className={inp} /></Row>
          <Row label="Location"><input value={data.recLocation} disabled onChange={e => u('recLocation', e.target.value)} className={inp + ' cursor-not-allowed opacity-60'} /></Row>
          <Row label="Personnel Name"><input value={data.recPersonnel} onChange={e => u('recPersonnel', e.target.value)} className={inp} /></Row>
          <Row label="Employee ID/NIK"><input value={data.recEmpId} onChange={e => u('recEmpId', e.target.value)} className={inp} /></Row>
          <Row label="Title"><input value={data.recTitle} onChange={e => u('recTitle', e.target.value)} className={inp} /></Row>
          <Row label="Status"><input value={data.recStatus} disabled onChange={e => u('recStatus', e.target.value)} className={inp + ' cursor-not-allowed opacity-60'} placeholder="KARYAWAN / KONTRAK / dll" /></Row>
        </div>
        ...
        <div className={grid2}>
          <Row label="Requester / Date"><input value={data.sigRequester} onChange={e => u('sigRequester', e.target.value)} className={inp} /></Row>
          <Row label="Sr. Mgr / Date"><input value={data.sigSrMgr} onChange={e => u('sigSrMgr', e.target.value)} className={inp} /></Row>
          <Row label="Spt. Dept. / Date"><input value={data.sigSptDept} onChange={e => u('sigSptDept', e.target.value)} className={inp} /></Row>
          <Row label="HOO / Date"><input value={data.sigHOO} onChange={e => u('sigHOO', e.target.value)} className={inp} /></Row>
          <Row label="Dept. Mgr / Date"><input value={data.sigDeptMgr} onChange={e => u('sigDeptMgr', e.target.value)} className={inp} /></Row>
          <Row label="IT Admin / Date"><input value={data.sigITAdmin} onChange={e => u('sigITAdmin', e.target.value)} className={inp + ' cursor-not-allowed opacity-60'} /></Row>
          <Row label="MSDI Mgr / Date"><input value={data.sigMSDIMgr} onChange={e => u('sigMSDIMgr', e.target.value)} className={inp + ' cursor-not-allowed opacity-60'} /></Row>
        </div>
